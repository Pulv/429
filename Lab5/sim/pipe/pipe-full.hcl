...
#-----------------------------Description---------------------------
#I added iaddq into this file. That was the only modification of the file. Allowed me to add an immediate with rB without having to do the two step (irmovq V, rB -> iaddq rA, rB).
#I did the same implementation as Part B.
####################################################################
#Stage:
#Fetch:
#       icode:ifun      <-- M1[PC]              #Read instruction byte
#       rA:rB           <-- M1[PC+1]            #Read register byte
#       valC            <-- M8[PC+2]            #Read immediate value
#       valP            <-- PC +10              ##Compute next program counter
#
#Decode:
#       valB            <-- R[rB]               #Read operand B
#
#Execute:
#       valE            <-- valB + valC         #Perform operation
#       Set CC                                  #Set condition codes post operation
#Memory:
#
#Write Back:
#       R[rB]           <-- valE                #Write back result (valE) into rB
#
#PC Update:
#       PC              <-- valP                #Update PC
####################################################################
